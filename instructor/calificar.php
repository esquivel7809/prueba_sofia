...
// PROCESAR CALIFICACIÓN (POST)
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['id'], $_POST['nota'])) {
        $id = $_POST['id'];
        $nota = floatval($_POST['nota']);
        $comentarios = $_POST['comentarios'] ?? '';

        // Validar que la entrega le pertenezca al instructor
        $checkStmt = $conn->prepare("
            SELECT au.id_actividad_user
            ...
            WHERE au.id_actividad_user = :id AND mf.id_instructor = :id_instructor
        ");
        $checkStmt->execute([
            ':id' => $id,
        ...
        if ($checkStmt->fetch()) {
            ...
            if (!is_numeric($nota) || $nota < 1.0 || $nota > 5.0) {
                ...
                header("Location: ver_entregas.php?msg=calificado");
                exit();
            }
        } else {
            // No autorizado
            header("Location: ver_entregas.php?msg=error");
            exit();
        }
    }
}

// MOSTRAR FORMULARIO DE CALIFICACIÓN (GET)
if (!isset($_GET['id'])) {
...
<body>
    ...
    <div class="container mt-4">
        ...
        <form method="POST" novalidate>
            <input type="hidden" name="id" value="<?php echo htmlspecialchars($entrega['id_actividad_user']); ?>">
            <div class="mb-3">
